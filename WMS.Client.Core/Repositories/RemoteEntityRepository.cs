...
namespace WMS.Client.Core.Repositories
{
    internal class RemoteEntityRepository<TEntity> : IEntityRepository where TEntity : EntityBase
    {
        ...
        public RemoteEntityRepository()
        {
            ...
            _hubConnection = new HubConnectionBuilder().WithUrl($"server/hub/{_type.Name}").WithAutomaticReconnect().Build();
            _hubConnection.On<TEntity>($"{_type.Name}Created", e => EntityCreated?.Invoke(this, new EntityCreatedEventArgs(e)));
            _hubConnection.On<TEntity>($"{_type.Name}Updated", e => EntityUpdated?.Invoke(this, new EntityUpdatedEventArgs(e)));
            _hubConnection.On<Guid>($"{_type.Name}Deleted", id => EntityDeleted?.Invoke(this, new EntityDeletedEventArgs(id)));
